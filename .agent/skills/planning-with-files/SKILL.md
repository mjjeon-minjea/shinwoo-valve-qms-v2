...
> 1. **저장경로 및 무력화**: 본 스킬의 기본인 `.planning/` 및 root 폴더 저장 대신 `C:\Users\mjjeon\Desktop\QMS 프로젝트\shinwoo-valve-qms\안티그래비티\task\` 폴더에 `YYYY-MM-DD_[과업주제]_R[N].md` 파일 명명 표준으로 리비전 파일 아카이빙을 강제한다.
> 2. **Hooks 무력화**: QMS 결재 및 DNAS 프로세스와의 충돌을 방지하기 위해 frontmatter 내의 모든 `hooks:` 정의를 전격 삭제하였으므로 자동 훅 실행은 발생하지 않는다.
> 3. **양식 및 언어**: 반드시 한글(한국어)로 작성하며, QMS DNAS 표준 task Step 형식으로 가공하여 출력한다.
