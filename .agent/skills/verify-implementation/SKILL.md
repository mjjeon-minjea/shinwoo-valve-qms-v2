...
> **[신우밸브 QMS 참고]** `/manage-skills`를 실행하면 새 검증 스킬이 자동으로
> 이 목록에 등록됩니다. Step 7의 n8n 헬스체크는 별도 등록 없이 항상 실행됩니다.
...
### Step 7: n8n 로컬 서버 헬스체크 및 통합 검증 워크플로우 확인

> **[신우밸브 QMS 필수 체크]** n8n 자동화 서버의 연동 상태를 확인합니다.
> 연결 실패 시 전체 검증을 중단하지 않고 Soft Warning만 출력합니다.

**대상:** `http://localhost:5678/healthz`
**검사 도구:** PowerShell `Invoke-WebRequest`

```powershell
# n8n 헬스체크 (Soft Warning - 실패 시 검증 중단 없음)
try {
    $response = Invoke-WebRequest -Uri "http://localhost:5678/healthz" `
        -TimeoutSec 5 -UseBasicParsing -ErrorAction Stop
    if ($response.StatusCode -eq 200) {
        Write-Host "✅ n8n 서버 정상 응답 (HTTP 200)"
        # 워크플로우 활성 상태 추가 확인
        $wf = Invoke-WebRequest -Uri "http://localhost:5678/api/v1/workflows" `
            -TimeoutSec 5 -UseBasicParsing -ErrorAction SilentlyContinue
        if ($wf -and $wf.StatusCode -eq 200) {
            Write-Host "✅ n8n 워크플로우 API 정상 응답"
        }
    }
} catch {
    Write-Host "⚠️ n8n 서버 미구동 상태 (설치 및 연동 대기 중)"
    Write-Host "   → 포트: 5678 / 엔드포인트: http://localhost:5678/healthz"
    Write-Host "   → 이 경고는 n8n 설치 전 정상 상태입니다. 검증을 계속 진행합니다."
}
```

**PASS 조건:** HTTP 200 응답 + 워크플로우 API 정상
**WARN 조건 (검증 계속):** 연결 거부, 타임아웃, 미설치 상태 → `⚠️ Soft Warning` 출력 후 진행
**FAIL 조건:** 없음 (n8n 미구동은 FAIL 처리하지 않음)
...
| 항목 | 상태 | 비고 |
|------|------|------|
| n8n 서버 응답 | ✅ PASS / ⚠️ WARN | HTTP 200 또는 미구동 경고 |
| 워크플로우 API | ✅ PASS / ⚠️ WARN | 활성 워크플로우 확인 |
```
