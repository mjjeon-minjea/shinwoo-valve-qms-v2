    ...
    const groupedData = useMemo(() => {
        ...
        data.forEach(row => {
            ...
            if (rowDate >= dateRange.start && rowDate <= dateRange.end) {
                const key = row.equipmentName || '미해당/미기재';
                if (!map[key]) {
                    map[key] = { eqName: key, records: 0, inspected: 0, passed: 0, failed: 0, resolved: 0, unresolved: 0 };
                }
                map[key].records++;
        ...
        <div className="space-y-6 flex flex-col min-h-screen text-slate-800 animate-fade-in p-2">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 px-4 pt-4">
                <div>
                    ...
                    <p className="text-slate-500 font-medium mt-1 ml-11">설비에 따른 검출량 및 품질 현황</p>
                </div>
                    ...
                    <div className="h-[350px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={groupedData.slice(0, 10)} margin={{ top: 20, right: 30, left: 20, bottom: 40 }}>
                                ...
                                <XAxis dataKey="eqName" angle={-45} textAnchor="end" height={60} interval={0} axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#64748b' }} />
                    ...
                    <div className="overflow-x-auto min-h-[300px]">
                        <table className="w-full text-sm text-left">
                            <thead className="text-xs uppercase bg-slate-50 text-slate-500 border-b border-slate-100">
                                <tr>
                                    <th className="px-6 py-4 font-bold tracking-wider">설비명</th>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {groupedData.map((row) => (
                                    <tr key={row.eqName} className="hover:bg-slate-50/50 transition-colors">
                                        <td className="px-6 py-4 font-bold text-slate-800">{row.eqName}</td>
