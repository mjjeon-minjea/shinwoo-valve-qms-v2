...
const NCRLedger = ({ user, onProcess }) => {
    ...
    const tabRows = tab === 'mine'
        ...
        : tab === 'done' ? rows.filter(isNcrFinished) : rows;
    const filtered = tabRows.filter(matchesQuery);
    const tabs = [
        { k: 'mine', label: `내 진행중 (${rows.filter(r => !isNcrFinished(r) && isNcrRelatedToUser(user, r)).length})` },
        { k: 'done', label: `완료 (${rows.filter(isNcrFinished).length})` },
        { k: 'all', label: `전체 (${rows.length})` }
    ];
    ...
    return (
        <div className="max-w-6xl">
            ...
            <p className="mt-3 text-xs text-slate-400">현재 탭 {filtered.length}건 표시 중 · 전수 {rows.length}건</p>

            {selected && (
                <NCRDetail report={selected} user={user}
                    readOnly
                    canProcess={myTurnV101(user, selected, settings)}
                    onProcess={(reportId) => { setSelected(null); onProcess?.(reportId); }}
