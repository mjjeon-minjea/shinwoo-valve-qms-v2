/* ─────────────────────────────────────────────────────────────────────────────
   InboundItems.jsx — 인수검사 「품목·부적합 관리」   (플랜 042 / P9 r9)

   r8 → r9 (차장 피드백 09-02)
     1. 이름이 「품목·부적합」 → **「품목·부적합 관리」** 가 됐다(사이드바·머리글 둘 다).
     2. 탭이 2개 → **3개**가 됐고, 대시보드와 **같은 조각**(ui.jsx 의 AreaBar)을 쓴다.
          · 품목 관리            — 품목유형 구성 · 품번 Top 10 · 품목 검색 표 · 규격외 측정치
          · 부적합 관리          — KPI · 부적합 유형 Top 10 · 업체별 부적합 Top 10 · 불합격 목록 전체
          · (09-02 r10) 「부적합 현황 조회(NCR)」 탭·NonConformanceStatus.jsx 는 과거 유물로 제거(아카이브).
        고른 탭은 localStorage `inbound_item_tab` 에 남는다.
        `initialTab="ncr"` 는 호환용 — 구 #inbound_status 링크는 「부적합 관리」 탭으로 간다.
     3. **기간 필터**가 붙었다(앞 두 탭에만 — NCR 탭에서는 그리지 않는다).
        기간은 대시보드·협력업체와 **같은 값**이다(localStorage `inbound_period`).
        「묶음」은 감춘다 — 이 화면엔 추이 차트가 없어 눌러도 아무 일이 안 일어난다.
     4. 표에 **모델명 · 사이즈** 칸이 생겼다. 품명 한 줄에서 뽑는다
        (lib/inboundStats.js 의 parseItem). 원래 품명은 그대로 옆 칸과 title 툴팁에 있다.
     5. 긴 목록은 **검색 + 20줄 쪽넘김**이다. 이 화면은 한 장에 안 들어가는 게 정상이고,
        페이지가 구르는 것도 정상이다(대시보드의 '스크롤 금지'는 그쪽 규칙이다).
     6. 랭킹 막대가 두 배로 두꺼워졌다 — 값은 ui.jsx · inbound.css 가 정한다.

   ※ 이 화면은 **보기 전용**이다. 자료를 고치는 칸은 하나도 없다.
   ※ 대장(inspections)과 측정값(inspection_measurements)은 출처가 다르다. 그래서
     "부적합 건수"와 "규격외 측정치 수"는 서로 맞지 않는다. 맞을 이유도 없다.
   ───────────────────────────────────────────────────────────────────────────── */
import React, { useEffect, useMemo, useRef, useState } from 'react';
import { RefreshCw, Search } from 'lucide-react';
import '../styles/inbound.css';
import {
    loadInspections, byItemType, byItemCode, topDefectTypes, bySupplier, summarize,
    dataSpan, rangeFilter, recentFails, parseItem, fmtRate, fmtPpm, ppm, PPM_TARGET,
...
import {
    ...
    GhostButton, AreaBar, GradeChip, Loading, ErrorCard, Empty,
    fmt, fx, pctText, gradeColor, useInboundTheme, useStickyString,
} from './inbound/ui';

const TOP_N = 10;
const PAGE = 20;                      /* 표 한 쪽에 20줄 — 스무 줄이면 눈이 한 번에 훑는다 */
...
/* ── 쪽넘김 ───────────────────────────────────────────────────────────────────
   페이지 번호를 다 그리면 30쪽짜리 표에서 줄이 두 줄이 된다. 앞뒤 2쪽과 처음·끝만
   그리고 나머지는 … 로 접는다. */
...
const Pager = ({ page, last, onGo, from, to, total }) => {
    if (last <= 1) {
        return (
            <div style={{ padding: '10px var(--ib-pad)', borderTop: '1px solid var(--ib-grid)', fontSize: 'calc(var(--ib-lbl)*.95)', color: 'var(--ib-ink4)' }}>
                <span className="tabular-nums">{fmt(total)}줄 전부</span>
            </div>
        );
    }
    const btn = (on) => ({
    ...
    return (
        <div className="flex flex-wrap items-center gap-2"
            style={{ padding: '10px var(--ib-pad)', borderTop: '1px solid var(--ib-grid)' }}>
            <span className="tabular-nums" style={{ fontSize: 'calc(var(--ib-lbl)*.95)', color: 'var(--ib-ink4)' }}>
                {fmt(from)}~{fmt(to)} / {fmt(total)}줄
            </span>
            <span className="flex-1" />
            <button type="button" onClick={() => onGo(page - 1)} disabled={page <= 1} style={{ ...btn(false), opacity: page <= 1 ? .45 : 1 }}>이전</button>
            {pageList(page, last).map((p, i) => (
                p === '…'
                    ? <span key={`d${i}`} style={{ color: 'var(--ib-ink4)', padding: '0 2px' }}>…</span>
                    : <button key={p} type="button" onClick={() => onGo(p)} className="tabular-nums" style={btn(p === page)}>{p}</button>
            ))}
            <button type="button" onClick={() => onGo(page + 1)} disabled={page >= last} style={{ ...btn(false), opacity: page >= last ? .45 : 1 }}>다음</button>
        </div>
    );
};

/** 검색어 + 쪽넘김을 한 벌로. 검색어가 바뀌면 1쪽으로 되돌린다. */
function usePaged(list, q, matches) {
    const [page, setPage] = useState(1);
    const hits = useMemo(() => {
        const s = String(q || '').trim().toLowerCase();
        if (s === '') return list;
        return list.filter((x) => matches(x, s));
    }, [list, q]);   // eslint-disable-line react-hooks/exhaustive-deps
    const last = Math.max(1, Math.ceil(hits.length / PAGE));
    const cur = Math.min(page, last);
    useEffect(() => { setPage(1); }, [q, list]);
    const from = hits.length === 0 ? 0 : (cur - 1) * PAGE + 1;
    const to = Math.min(cur * PAGE, hits.length);
    return {
        hits, page: cur, last, from, to,
        rows: hits.slice((cur - 1) * PAGE, cur * PAGE),
...
const InboundItems = ({ initialTab }) => {
    ...
    /* 구 #inbound_status 링크로 들어오면 세 번째 탭으로. 저장값보다 이게 이긴다(한 번만). */
    useEffect(() => {
...
const Body = ({ st, tab }) => {
    ...
    const TOPITEM = useMemo(
        () => ITEMS.slice().sort((a, b) => b.count - a.count || b.inQty - a.inQty).slice(0, TOP_N),
        [ITEMS],
    );
            ...
            {F.length === 0 ? (
                ...
                <>
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4" style={{ marginBottom: 16 }}>
                        ...
                        <KpiTile delay={0.11} label="품번" value={ITEMS.length} unit="개"
                            footnote={`부적합이 있던 품번 ${fmt(ITEMS.filter((o) => o.ngCount > 0).length)}개`} />
                    ...
                    <div className="grid grid-cols-12 gap-4">
                        {/* A. 품목유형 구성 */}
                        <Card className="col-span-12" delay={0.20}>
                        ...
                        {/* B. 품번 Top 10 (검사 건수) */}
                        <Card className="col-span-12" delay={0.24}>
                            <SectionTitle title={`품번 Top ${TOP_N}`} subtitle="검사 건수 순 · 우측은 그 품번의 불량률(% · PPM)" />
                            <div style={{ marginTop: 12 }}>
                                {TOPITEM.length === 0 ? <Empty t="이 기간에 입고된 품번이 없다" /> : (
                                    <RankBar rows={TOPITEM.map((o) => ({ ...o, name: o.key }))} valueKey="count" valueUnit="건"
                                        nameWidth={190}
                                        rightOf={(r) => (
                            ...
                            <ul className="list-none m-0 p-0" style={{ marginTop: 10, fontSize: 'calc(var(--ib-lbl)*.95)', color: 'var(--ib-ink4)' }}>
                                {TOPITEM.slice(0, 3).map((o) => (
                                    <li key={o.key} className="truncate" title={o.itemName}>
                                        {o.key} — {o.model} · {o.size} · {o.itemName || '(품명 없음)'}
                                    </li>
                                ))}
                            </ul>
                        </Card>
                        ...
                        {/* C. 품목 검색 표 */}
                        <Card className="col-span-12" delay={0.28} style={{ padding: 0 }}>
                            ...
                            {P1.hits.length === 0 ? <Empty t="찾는 품목이 없다" /> : (
                                <>
                                    ...
                                    <Pager page={P1.page} last={P1.last} onGo={P1.go} from={P1.from} to={P1.to} total={P1.hits.length} />
                        ...
                        {/* D. 규격외 측정치 */}
                        <Card className="col-span-12" delay={0.32} style={{ padding: 0 }}>
                            <div style={{ padding: 'var(--ib-pad) var(--ib-pad) 12px' }}>
                                <SectionTitle title="규격외 측정치"
                            ...
                            {st.spcErr ? (
                                ...
                                <>
                                    ...
                                    <Pager page={P3.page} last={P3.last} onGo={P3.go} from={P3.from} to={P3.to} total={P3.hits.length} />
                                    <div style={{ padding: '0 var(--ib-pad) 14px', fontSize: 'calc(var(--ib-lbl)*.95)', color: 'var(--ib-ink4)', lineHeight: 1.5 }}>
                ...
                <>
                    ...
                    <div className="grid grid-cols-12 gap-4">
                        ...
                        <Card className="col-span-12" delay={0.28} style={{ padding: 0 }}>
                            <div style={{ padding: 'var(--ib-pad) var(--ib-pad) 12px' }}>
                                <SectionTitle title="불합격 목록 전체"
                                    subtitle={`이 기간(${periodText}) 판정 불합격 · 최신순 · 보기 전용`}
