...
import { 
    PackageSearch, RefreshCw, Table2
} from 'lucide-react';
import { 
    ResponsiveContainer,
    PieChart,
    ...
    const groupedData = useMemo(() => {
        ...
        data.forEach(row => {
            ...
            if (rowDate >= dateRange.start && rowDate <= dateRange.end) {
                const key = row.modelCategory || '미기재';
                if (!map[key]) {
                    map[key] = { modelCatName: key, records: 0, inspected: 0, passed: 0, failed: 0, resolved: 0, unresolved: 0 };
                }
                map[key].records++;
                    ...
                    <div className="overflow-x-auto min-h-[300px]">
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {groupedData.map((row) => (
                                    <tr key={row.modelCatName} className="hover:bg-slate-50/50 transition-colors">
                                        <td className="px-6 py-4 font-bold text-slate-800">{row.modelCatName}</td>
...
export default ModelCategoryAnalysis;
