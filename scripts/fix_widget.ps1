...
$ps1Path = Join-Path $desktop "qms_widget_ui.ps1"
$vbsPath = Join-Path $desktop "QMS_위젯_실행.vbs"
...
'@
[IO.File]::WriteAllText($ps1Path, $ps1Code, [System.Text.Encoding]::UTF8)

$vbsCode = 'Set objShell = CreateObject("WScript.Shell")' + "`r`n"
$vbsCode += 'objShell.Run "powershell.exe -WindowStyle Hidden -ExecutionPolicy Bypass -File """ & "' + $ps1Path + '"" & """", 0, False' + "`r`n"
[IO.File]::WriteAllText($vbsPath, $vbsCode, [System.Text.Encoding]::UTF8)

# Delete old broken files
$oldFiles = @("QMS_서버위젯.ps1", "QMS_서버위젯_실행.bat")
foreach ($f in $oldFiles) {
    Remove-Item (Join-Path $desktop $f) -ErrorAction SilentlyContinue
}
