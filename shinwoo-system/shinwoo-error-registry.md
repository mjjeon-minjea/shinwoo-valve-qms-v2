...
- **실수**: 진짜 전역 규칙 기본 경로(`C:\Users\mjjeon\.gemini\GEMINI.md`)와 로컬 워크스페이스 내 규칙 파일(`.agent/rules/GEMINI.md`)의 실질적 연동 범위를 혼동하여 차장님께 아키텍처 혼선을 드림.
- **원인**: IDE 자체 전역 물리 보관 경로에만 몰두하여, 실제 안티그래비티 구동 시 런타임 행동을 구속하고 주입받는 룰북의 원천 소스가 워크스페이스 내부의 `.agent/rules/GEMINI.md`라는 팩트를 망각함.
- **영향**: 차장님의 아키텍처 설안을 이해하지 못하고 엉뚱한 조항 매핑 해설을 대화창에 도배함.
- **재발 방지 규칙**: 규칙 연동 및 정합성 설계 시, 임의의 추측을 전면 금지하고 반드시 현재 에이전트의 구동 세션 런타임 로드 파일 및 구조도를 1순위로 교차 대조할 것.

### ERR-018
- **날짜**: 2026-06-02 (대화 77d4f8f9)
- **실수**: 안티그래비티 2.0이 기동 시 워크스페이스 내에서 자동으로 감지 및 로딩하는 파일명이 `GEMINI.md`로 하드코딩 고정되어 있다는 런타임 팩트를 망각하고, 자의적으로 파일명을 `qms_rules.md`로 변경 생성함.
- **원인**: 규칙의 로컬 이격 설계(상속 선언) 목적에만 휩쓸려 에이전트 자체의 하드웨어/소프트웨어적 강제 로딩 규칙을 체크하지 않음.
- **영향**: 룰셋 로딩 장애를 유발하여 에이전트의 헌법 제어(승인 키워드 락 등) 장치를 전면 무력화할 뻔함.
- **재발 방지 규칙**: 에이전트가 자동 인식하여 읽어 들이는 예약된 핵심 파일명(`GEMINI.md` 등)은 절대 변형하지 말고, 알맹이 내용만 상속문 및 SOP 내비게이터로 이격하여 결합할 것.

### ERR-019
- **날짜**: 2026-06-06 (대화 6429d07d-69a5-4b4b-8a69-3e4979da775c)
- **실수**: 마이그레이션 PowerShell 스크립트 실행 시 한글 인코딩 깨짐 현상으로 인해 ParserError(UnexpectedToken)가 발생하여 구동 실패함
- **원인**: 시스템 한글 인코딩(ANSI vs UTF-8) 충돌로 인해 스크립트 파일 내의 "년", "월", "일", "안티그래비티" 등의 한글 문자열이 파서에 의해 깨져 구문 파괴를 유발함
- **영향**: 1차 마이그레이션 정돈 작업 일시 실패 및 대기 시간 발생
- **재발 방지 규칙**: PowerShell 스크립트 작성 시 한글 리터럴 문자열 사용을 원천 배제하고, 아스키 유니코드 결합형 `[char]50504` 등의 숫자 코드포인트를 조합하여 한글 변수를 선언함으로써 인코딩 환경과 상관없이 100% 무결한 기동을 담보하도록 우회 조치함.
