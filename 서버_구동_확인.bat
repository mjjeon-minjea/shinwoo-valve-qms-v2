...
for /f "tokens=*" %%a in ('netstat -ano ^| findstr "LISTENING" ^| findstr ":5173"') do set FRONTEND_RUNNING=1
...
for /f "tokens=*" %%a in ('netstat -ano ^| findstr "LISTENING" ^| findstr ":3001"') do set BACKEND_RUNNING=1
