---
title: "QMS v2 5단계 점진적 하네스 이식 기획안 (R4 - 최종 완결판)"
type: "구현 계획서 (Implementation Plan)"
source_folder: "안티그래비티/plan"
source_file: "2026-05-27_QMS_v2_점진적_하네스_이식_기획안_R4.md"
date: "2026-05-27"
revision: "R4"
...
* **갱신 작성일:** 2026-05-27  
* **문서 버전:** R4 (최종 완결판)  
* **보관 경로:** `안티그래비티/plan/2026-05-27_QMS_v2_점진적_하네스_이식_기획안_R4.md`
...
* **목표:** 원격 Supabase 환경 및 쿼리 구조를 반영하여, 런타임에 실패하지 않는 원격 테이블 존재성 검증을 현실화합니다.
* **구현 사양:**
  ...
  - **프론트엔드 URL 호출 스캔 (다중 세그먼트 캡처):** `src\components\` 내의 모든 UI 컴포넌트들을 Node.js 정적 스캐너로 스캔하여, 컴포넌트들이 `api.fetch('...')` 형태로 호출하는 **경로 내의 테이블명 목록**을 정교한 RegEx를 활용하여 추출합니다.
    - *다중 세그먼트 캡처 정규식:* `api\.fetch\(\s*['"]\/([^/'"]+?)[/'"]?`
    - `/settings/global`과 같이 깊은 동적 라우팅 경로에서도 첫 번째 슬래시 다음의 세그먼트인 `settings` 테이블만 정확하게 추출해 내도록 캡처 룰을 구성합니다.
