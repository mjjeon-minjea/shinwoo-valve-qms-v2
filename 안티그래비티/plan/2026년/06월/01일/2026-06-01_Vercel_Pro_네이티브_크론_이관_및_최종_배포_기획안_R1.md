# 📋 Vercel Pro 네이티브 크론 이관 및 메인웹 최종 배포 구현 계획서 (R1)
...
특히 **Claude(코워크) 비서의 날카로운 R0 검토보고서 피드백**에 근거하여, 환경변수 4대 키의 명시적 파이프라인 검증 단계 추가, 테스트 Vercel의 Pro 플랜 소속 여부 사전 조회, 그리고 원격 GitHub Actions Workflow 파일의 존재 여부 사전 인스펙션 절차를 추가하여 **R1 수정본**으로 개정 및 보완하였습니다.
...
> [!IMPORTANT]
> **1. GitHub Actions 우회 크론 Workflow 원격 존재 여부 사전 인스펙션 및 수동 비활성화 (Human-controlled)**
> * 현재 로컬 코드베이스 및 Git 이력 실사 결과, `.github/workflows/` 디렉토리는 로컬에 물리적으로 존재하지 않는 것으로 확인되었습니다.
> * **[R1 보완 사항]:** 안티그래비티가 배포 착수 직전 원격 저장소(`test-origin`, `origin`)의 브랜치 원격 리스트를 정밀 추적하여 GitHub 상에 실제 우회 Workflow가 활성화되어 있는지 선제 검사하고 보고해 올리겠습니다.
> * 원격에 실제 Workflow가 존재할 시, 원격의 불필요한 중복 호출 및 과금 방지를 위해 GitHub 웹 UI 상에서 **`[Actions] ➔ [해당 우회 Workflow 선택] ➔ [...] ➔ [Disable Workflow]`** 설정을 통해 수동 비활성화해 주시는 것을 청구합니다.
...
* **내용:** 원격 브랜치 실사 검증을 통해 파일(`.github/workflows/*.yml`) 존재를 확정하고, 원격 존재 시 GitHub 웹 콘솔 상에서 수동 비활성화(Disable) 조치를 유도 및 가이드합니다.
...
* **[R1 신규 검증 절차]**
  - **테스트 Vercel Pro 팀 소속 조회:** 배포 전 안티그래비티가 `shinwoo-valve-qms` 프로젝트의 요금제 상태를 조회하여 Pro 등급 10분 크론 정상 수용 상태인지 사전 검수 보고합니다.
  - **Vercel Pro 환경변수 4대 키 사전 검사:** `GOOGLE_SHEETS_CSV_URL`, `VITE_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `GEMINI_API_KEY` 4개 환경변수가 Vercel Pro Project Settings에 무결하게 등록되어 활성화 중인지 콘솔 상의 존재 여부를 크로스체크하여 완벽한 동작을 확정합니다.
...
2. 배포 착수 직전 원격 브랜치 분석을 수행하여 GitHub Actions Workflow 원격 존재 여부 정밀 실사 및 비활성화 가이드 작성.
