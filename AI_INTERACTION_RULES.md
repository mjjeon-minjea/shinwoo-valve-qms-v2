...
## 📌 제5장. 데이터 최적화 및 토큰 관리 (Token Efficiency)

- **[대용량 데이터 무시 원칙]**: `db.json` 파일을 직접 읽거나 분석할 때, 하단의 `process_inspections` 및 `receiving_inspections` 섹션은 내용 파악을 위한 최소한의 구조(Keys)만 확인하고, 상세 데이터(Row 데이터)는 읽지 않는다. 이는 토큰 낭비를 방지하고 응답 속도를 높이기 위함이다.
- **[전수 읽기 제한]**: 프로젝트 구조 파악 시 대용량 데이터 파일(`*.json`, `*.csv`, `*.xlsx`)은 차장님의 명시적 지시가 없는 한 전수 읽기를 자제하며, 필요 시 상단 일부(Header)만 확인한다.
- **[효율적 보고]**: 데이터 분석 결과 보고 시 수만 건의 로우 데이터를 나열하지 않고, 핵심 통계치(합계, 불량률 등)와 결론 위주로 요약하여 보고한다.
