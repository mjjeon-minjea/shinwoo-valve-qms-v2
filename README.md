...
2. **환경변수 셋업 (`.env.local` 작성)**
   ...
   _주의: API 중앙 설정 상수(`LOCAL_API_URL`)의 변경이 필요한 경우 `VITE_LOCAL_API_URL` 값을 지정하십시오._
...
3. **로컬 개발 서버 기동 (기본 포트 5173 고수)**
   ...
   - _중요:_ 안티그래비티 자체 브라우저 뷰어가 프록시로 정상 스트리밍하여 화면이 뻗지 않도록 Vite 기본 포트인 **5173**을 절대 고수해 주십시오.
